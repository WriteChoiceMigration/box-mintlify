...
埋め込みURLをAPIから直接生成する場合は、[ファイル情報の取得エンドポイント](/ja/reference/get_files_id)を使用して、`fields`パラメータを介して`expiring_embed_link`をリクエストします。
...
高度なプレビューのカスタマイズ機能やイベント処理機能を活用するには、Box [UI Preview Element](/ja/guides/embed/ui-elements/preview)を使用します。

このプレビューElementを設定するには、まず、必須コンポーネントをインストールします。

<Card href="/ja/guides/embed/ui-elements/installation" title="Box Elementsとプレビューのインストール"/>
...
プライマリアクセストークンの高度な権限により、JavaScriptコードでは、ダウンスコープされたバージョンのトークンを使用することを強くお勧めします。[ダウンスコープのベストプラクティス](/ja/guides/embed/box-view/best-practices#use-downscoped-tokens)を参照してください。

</Warning>

[uie]: /ja/guides/embed/ui-elements
