...
新しいファイルのチェックを実行するには、実際のファイルをアップロードする場合と同じパラメータ (バイナリコンテンツを除く) を使用して[`OPTIONS /files/content`](/ja/reference/options_files_content) APIを呼び出してください。
...
新しいファイルバージョンのチェックを実行するには、実際のファイルをアップロードする場合と同じパラメータ (バイナリコンテンツを除く) を使用して[`OPTIONS /files/:id/content`](/ja/reference/options_files_content) APIを呼び出してください。
...
[chunked]: /ja/guides/uploads/chunked

[chunkedsession]: /ja/guides/uploads/chunked/create-session
