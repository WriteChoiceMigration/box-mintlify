...
```json
{
  ...
  "help_url": "/reference/error-codes/invalid-api-version",
...
Box-API-Deprecated-Reason: /ja/reference/deprecated
...
公式サポートが終了した古いAPIバージョン (`2025.0`など) を使用しようとすると、レスポンスでは`HTTP error code 404 - Not Found`が返されます。詳細については、[URLでの正しくないAPIバージョンの呼び出し](#urlでの正しくないapiバージョンの呼び出し)を参照してください。
