...
Boxユーザーオブジェクトの最上位を探索すると、SSOサービスのユーザーオブジェクトからBox[ユーザーオブジェクト](/ja/reference/resources/user)に一意の識別子を追加して双方を関連付けるために使用できるオプションがわかります。
...
* メールアドレスによる関連付けを実行できるのは[管理対象ユーザー](https://developer.box.com/platform/user-types/#managed-users)のみです。[App User](https://developer.box.com/platform/user-types/#app-user)にはBoxによって自動的にメールアドレスが割り当てられるため、`login`をSSOサービスのメールアドレスになるよう割り当てることはできません。
...
* [App User](https://developer.box.com/platform/user-types/#app-user)ではなく[管理対象ユーザー](https://developer.box.com/platform/user-types/#managed-users)タイプのみが使用されている。
