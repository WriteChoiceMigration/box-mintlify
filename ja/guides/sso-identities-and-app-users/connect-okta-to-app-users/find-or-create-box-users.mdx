---
title: "Box App Userの検索または作成"
sidebarTitle: 5. Box App Userの検索または作成
type: quick-start
...
Oktaユーザーの関連付けられたBoxユーザーが検出されたら、明確に[そのユーザーのスコープに設定された](/ja/guides/authentication/jwt/user-access-tokens)アクセストークンを生成してBox APIコールを実行します。その後、現在のユーザーを取得するための呼び出しを実行して、すべてが機能していることと有効なユーザーアクセストークンがあることを確認します。
