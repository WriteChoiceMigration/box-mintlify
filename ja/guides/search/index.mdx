...
検索APIで使用できる各種機能の詳細については、[リファレンスドキュメント](/ja/reference/get_search)を参照してください。

</Note>

## クエリ演算子

検索APIは、`AND`、`OR`、`NOT`、`""`など、いくつかの[検索演算子](/ja/guides/search/query-operators)をサポートします。これらの演算子を使用すると、より複雑な組み合わせの検索語句に一致する項目のみが返されるように検索結果を絞り込むことができます。
...
<Card href="/guides/search/query-operators" title="論理演算子の使用の詳細を確認する"/>
...
<Card href="/guides/search/indexing" title="Boxの検索インデックスの詳細を確認する"/>
...
<Card href="/guides/metadata/queries/comparison" title="メタデータクエリAPIの詳細を確認する"/>

[mdq]: /ja/guides/metadata/queries

[mdq_api]: /ja/reference/post_metadata_queries_execute_read
