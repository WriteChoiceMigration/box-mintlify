...
* 目的のファイルタイプ`[pdf]`を表す`x-rep-hints`ヘッダーを渡して、[PDFをリクエスト](/ja/guides/representations/request-a-representation)する
* `url_template`を呼び出して[PDFをダウンロード](/ja/guides/representations/download-a-representation)する。その際、`{+asset_path}`は、リクエストするPDFのページ (`1.pdf`など) に置き換えます。
