...
See the [Client Error resource](/reference/resources/client_error) for more details.
...
| **Solution** | Your application tried to access the Box API from a website. The application needs to [explicitly allow Cross Origin Resource Sharing](/guides/security/cors/#enabling-cors-for-your-domain) for the domain your site is hosted on.    |
...
| **Solution** | The client is performing operations too quickly and has been rate limited. Client is advised to retry their request after the amount of time specified by the `retry-after` header. There are [four rate limits](/guides/api-calls/permissions-and-errors/rate-limits) to be aware of. |
