...
[`OPTIONS /files/content`](/reference/options_files_content) API with the same
...
[`OPTIONS /files/:id/content`](/reference/options_files_content) API with the same
parameters (except for the binary content) as if uploading an actual file.
...
[chunkedsession]: /guides/uploads/chunked/create-session
