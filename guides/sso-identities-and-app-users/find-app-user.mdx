...
[create a user access token](/guides/authentication/jwt/user-access-tokens),
or make [as user calls](/guides/authentication/jwt/as-user), to access Box
APIs as that user.

If a Box user is not found you should create a new user with an association to
the SSO user record.

To search for existing users the [List Enterprise Users](/reference/get-users)
