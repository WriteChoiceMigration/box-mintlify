...
[`POST /folders`](/reference/post_folders) API.

<Note>

When using server-side JWT authentication, we recommend creating folder that
is owned by the application's
[Service Account](https://developer.box.com/platform/user-types/#service-account). This way,
...
[invite](/reference/post-collaborations) a managed Box user to the folder before
...
[`POST /file-requests/:id/copy`](/reference/post_file_requests_id_copy)
...
to a new folder [with one API call](/guides/file-requests/copy).
